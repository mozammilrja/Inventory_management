...
import { fetchDashboardAnalyticsAsync } from "@/lib/store/slices/productSlice";
import {
  Card,
  CardHeader,
  CardTitle,
...
export default function DashboardPage() {
  ...
  const { products, dashboardStats, isLoadingAnalytics, error } =
    useAppSelector((s: any) => s.product);
  ...
  // ✅ OPTIMIZATION 2: Memoized data fetcher
  const fetchData = useCallback(
    async (filters?: string) => {
      try {
        await dispatch(fetchDashboardAnalyticsAsync(filters));
  ...
  // ✅ OPTIMIZATION 4: Debounced filter effect - much cleaner
  useEffect(() => {
    const params = new URLSearchParams();
    if (debouncedDept !== "All") params.append("department", debouncedDept);
    if (debouncedAssetType !== "All")
      params.append("assetType", debouncedAssetType);
    if (debouncedStatus !== "All") params.append("status", debouncedStatus);
    if (debouncedCondition !== "All")
      params.append("condition", debouncedCondition);
    if (debouncedRange.length === 2) {
      params.append("startDate", debouncedRange[0].toISOString());
      params.append("endDate", debouncedRange[1].toISOString());
    }

    const queryString = params.toString();
    fetchData(queryString ? `?${queryString}` : undefined);
  }, [
    debouncedDept,
    debouncedAssetType,
    debouncedStatus,
      ...
      Array.from(
        new Set(
          products
            ...
            .filter(
              (d: any): d is string => typeof d === "string" && d.trim() !== ""
            )
        )
      ),
    [products]
  ...
  // ✅ OPTIMIZATION 6: Memoized dashboard stats for charts
  const memoizedStats = useMemo(() => {
    if (!dashboardStats) return null;

    return {
      ...dashboardStats,
      // Add any computed values here if needed
    };
  }, [dashboardStats]);
          ...
          <CardContent>
            {!hasData || safeStats.trendData.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={220}>
                <AreaChart data={safeStats.trendData}>
                  <defs>
